...
@php
    $isAvailable = ($book->stock ?? $book->available_copies ?? 0) > 0;
    $status = $isAvailable ? 'available' : 'borrowed';

    // Cover bisa berupa URL eksternal (http/https) ATAU path file lokal di storage
    $cover = $book->cover
        ? (str_starts_with($book->cover, 'http') ? $book->cover : asset('storage/' . $book->cover))
...
<div class="bg-white rounded-2xl border border-stone-100 shadow-sm p-4 flex flex-col">
    <a href="{{ route('member.catalog.show', $book) }}" class="relative mb-4 block">
        <img src="{{ $cover }}" alt="{{ $book->title }}"
             class="w-full h-56 object-cover rounded-xl bg-stone-100"
             onerror="this.onerror=null;this.src='https://placehold.co/300x360/2f4f3f/e8dcc4?text={{ urlencode($book->title) }}';">
        <x-badge-status :status="$status" class="absolute top-3 left-3" />
    </a>
